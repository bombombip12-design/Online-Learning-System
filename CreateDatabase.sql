...
BEGIN
    CREATE TABLE Classes (
	    ...
	    Status NVARCHAR(50) Check (Status in ('Active', 'Non-active')),
...
-- Tạo bảng SubmissionsAttachments
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'SubmissionsAttachments')
BEGIN
    CREATE TABLE SubmissionsAttachments (
        Id INT IDENTITY(1,1) PRIMARY KEY,
        ...
        CONSTRAINT FK_SubmissionsAttachments_Submissions FOREIGN KEY (SubmissionId) REFERENCES Submissions(Id) ON DELETE CASCADE
    );
END
GO
